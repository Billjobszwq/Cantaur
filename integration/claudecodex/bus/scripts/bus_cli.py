...
PROTOCOL_VALIDATOR_PATH = ROOT.parent / "protocols" / "scripts" / "validate_openclaw_a2a.py"
VALID_MESSAGE_TYPES = {
    "TASK",
    "RESULT",
    "REVIEW",
...
def strict_schema_enabled() -> bool:
    raw = os.getenv("OPENCLAW_A2A_STRICT_SCHEMA", "1").strip().lower()
...
def protocol_validator_module():
    ...
    spec = importlib.util.spec_from_file_location("validate_openclaw_a2a", str(PROTOCOL_VALIDATOR_PATH))
...
def validate_message(msg: dict[str, Any]) -> None:
    ...
    ensure(msg["protocol"] == "openclaw-a2a/v1", "protocol invalid")
