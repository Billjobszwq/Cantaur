...
当前也已经有对应消费器：
- `${OPENCLAW_HOME}/workspace/scripts/main_knowledge_message_consumer.py`
...
- 通过 `${OPENCLAW_HOME}/workspace/scripts/agent_task_result_bridge.py` 产出标准 `RESULT`
- `RESULT` 可以重新进入 `bus_messages`
- 再由 `${OPENCLAW_HOME}/workspace/scripts/main_result_feedback_consumer.py` 消费并回流成新的 `KNOWLEDGE_PAGE_UPDATED`
...
- 规则文件：
  - `${OPENCLAW_HOME}/workspace/knowledge/schemas/agent-knowledge-subscriptions.v1.json`
- 已验证接收方：
  - `research`
  - `ops`

已验证接入点：
- `${OPENCLAW_HOME}/workspace/scripts/knowledge_base.py`

已验证运行面：
- `${OPENCLAW_HOME}/workspace/integration/claudecodex/runtime/shadow-live/`
...
```bash
${OPENCLAW_HOME}/workspace/integration/claudecodex/bus/agent-bus.db
```

运行目录：

```bash
${OPENCLAW_HOME}/workspace/integration/claudecodex/bus/runtime
```
...
```bash
python3 ${OPENCLAW_HOME}/workspace/integration/claudecodex/bus/scripts/bus_cli.py \
  init \
  ${OPENCLAW_HOME}/workspace/integration/claudecodex/bus/agent-bus.db \
  ${OPENCLAW_HOME}/workspace/integration/claudecodex/bus/runtime
```
...
python3 ${OPENCLAW_HOME}/workspace/integration/claudecodex/bus/scripts/bus_cli.py \
  ...
  ${OPENCLAW_HOME}/workspace/integration/claudecodex/bus/agent-bus.db \
  ${OPENCLAW_HOME}/workspace/integration/claudecodex/bus/runtime \
  ${OPENCLAW_HOME}/workspace/integration/claudecodex/protocols/examples/task.example.json
```
