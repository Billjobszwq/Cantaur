...
PROFILE_PATH = LIVE_LINK_ROOT / "config" / "real-version-release-candidate.v1.json"
...
def ensure_runtime_dirs(runtime_name: str) -> dict[str, Path]:
    root = RUNTIME_BASE / runtime_name / "real-version-release-candidate"
    dirs = {
        "root": root,
...
def profile_status() -> dict[str, Any]:
    ...
    return {
        ...
        "active": active,
        "mode": "formal_fusion_default_status",
    }
...
def cmd_activate(actor: str) -> int:
    ...
    append_jsonl(dirs["audit"] / "release-candidate-audit.jsonl", {"event": "activate", "record": str(record_path), "created_at": now_iso()})
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    append_jsonl(
        dirs["audit"] / "release-candidate-audit.jsonl",
    ...
    result = {
        ...
        "unique_fusion": unique_sync,
        "mode": "formal_fusion_default_entry",
    }
    print(json.dumps(result, ensure_ascii=False, indent=2))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="formal fusion default entry")
    ...
    p_run.add_argument("--decision-note", default="正式融合默认版下，允许当前任务进入 Phase 32 默认工作面。")
    p_run.add_argument("--bridge-reason", default="当前任务进入正式融合默认版，使用 Phase 32 作为默认长期工作面。")
