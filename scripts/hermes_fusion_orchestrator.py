...
POLICY_PATH = KNOWLEDGE_ROOT / "schemas" / "hermes-fusion-autopilot.v1.json"
ROLLOUT_POLICY_PATH = KNOWLEDGE_ROOT / "schemas" / "hermes-fusion-rollout.v1.json"
LIFECYCLE_SCRIPT = WORKSPACE_ROOT / "integration" / "claudecodex" / "live-link" / "scripts" / "main_bridge_lifecycle.py"
...
def default_policy() -> dict[str, Any]:
    return {
        "version": "hermes-fusion-autopilot/v1",
        "updated_at": now_iso(),
        "knowledge": {
            "enabled": True,
            "decision": "observe",
            ...
            "reviewer": "main",
            "note": "auto triage by hermes fusion orchestrator",
        },
        "memory": {
            "enabled": True,
            "status": "approved",
...
def default_rollout_policy() -> dict[str, Any]:
    return {
        "version": "hermes-fusion-rollout/v1",
        "updated_at": now_iso(),
        "active_tier": "low",
        "tiers": {
            "off": {"enabled": False},
...
def lifecycle_timeout_scan(runtime: str, timeout_minutes: int, limit: int, actor: str = "hermes-fusion") -> dict[str, Any]:
    return run_json(
        [
            "/usr/bin/python3",
            str(LIFECYCLE_SCRIPT),
...
def apply_knowledge_autopilot(
    ...
    note = str(cfg.get("note", "auto triage by hermes fusion orchestrator"))
    ...
    try:
        ...
        if callable(register):
            for existing in sorted(REPORT_ROOT.glob("*.md")):
                ...
                register(title, existing, "hermes fusion orchestration report")
    except Exception:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Hermes-style orchestration layer for OpenClaw memory+knowledge loop")
